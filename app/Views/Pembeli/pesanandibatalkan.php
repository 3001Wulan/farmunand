...
<html lang="id">
  ...
  <title>Pesanan Dibatalkan - FarmUnand</title>
  ...
  <body>
    ...
    <div class="content">
      ...
      <div class="page-header">
        ...
        <div class="d-none d-md-block small">Daftar pesanan yang telah dibatalkan</div>
      </div>
      ...
      <div class="card-container">
        ...
        <div class="mb-3 d-flex flex-wrap tabs-wrap">
          ...
          <a href="/pesanandibatalkan"  class="btn btn-sm btn-success btn-filter active">Dibatalkan</a>
          <a href="<?= base_url('penilaian/daftar') ?>" class="btn btn-sm btn-outline-success btn-filter">Berikan Penilaian</a>
        </div>

        <!-- Loop Pesanan Dibatalkan -->
        <?php if (!empty($orders)): ?>
            ...
            <div class="card order-card">
              <div class="card-body d-flex flex-wrap justify-content-between align-items-center">
                <div class="d-flex align-items-center">
                  <div class="order-img">
                    <?php if (!empty($order['foto'])): ?>
                      <img src="<?= base_url('uploads/produk/'.$order['foto']); ?>" alt="<?= esc($order['nama_produk'] ?? 'Produk'); ?>">
                    <?php else: ?>
                ...
                <div class="text-end mt-3 mt-md-0">
                  <p class="mb-1 status text-danger"><?= esc($order['status_pemesanan'] ?? 'Dibatalkan') ?></p>
                  <p class="mb-0">
                    Total Pesanan
                    <span class="fw-bold">
                      <?php
                        $qty   = (int)($order['jumlah_produk'] ?? 0);
                        $harga = (float)($order['harga'] ?? 0);
                        $total = isset($order['total_harga']) ? (float)$order['total_harga'] : ($harga * $qty);
                      ?>
          ...
          <?php endforeach; ?>
        <?php else: ?>
          <div class="alert alert-info mb-0">Tidak ada pesanan yang dibatalkan.</div>
        <?php endif; ?>
      </div>
    </div>
