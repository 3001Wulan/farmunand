<div class="sidebar d-flex flex-column">
  ...
  <div class="d-grid gap-1 px-3 flex-grow-2">
    <a href="<?= base_url('profile') ?>"
      class="sidebar-link <?= url_is('profile*') ? 'active' : '' ?>">
      Akun Saya
    </a>
    <a href="<?= base_url('dashboarduser') ?>"
      class="sidebar-link <?= url_is('dashboarduser*') ? 'active' : '' ?>">
      Dashboard
    </a>
    <a href="<?= base_url('keranjang') ?>"
      class="sidebar-link <?= url_is('keranjang*') ? 'active' : '' ?>">
      Keranjang
      <?php $cartCount = session()->get('cart_count_u_' . ($user['id_user'] ?? 0)) ?? 0; ?>
      <span class="badge bg-danger ms-2"><?= (int)$cartCount ?></span>
    </a>
    <a href="<?= base_url('riwayatpesanan') ?>"
      class="sidebar-link <?= url_is('riwayatpesanan*') ? 'active' : '' ?>">
      Pesanan Saya
    </a>
  </div>
